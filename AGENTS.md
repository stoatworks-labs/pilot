...
- **Never loaded into Resolume**, and never installed into Arena. How the
  parameters *present* — four groups in the inspector, two eight-entry colour
...
- **The Windows build has never been compiled.** CI is written and has never run;
  there is no repo for it to run in.
...
- **No OpenFX port, no browser demo, no user guide, no factory presets, no `--pipe`
  frame-streaming mode.** None of them are started.
- **`StoatworksAbout.h` and `ATTRIBUTIONS.md` are provisional hand copies**, written
  in the shape `stoatworks-backend/scripts/sync-about.py` produces, with
  `guide = ""` because no user guide exists — so the About block shows three
  buttons rather than four. graticule shipped the same way.
...
- MIT, copyright Stoatworks Labs. Intended home
  `github.com/stoatworks-labs/pilot`, which does not exist yet.
